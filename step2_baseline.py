...
for name, model in models.items():
    ...
    try:
        ...
        if f1_macro > best_f1:
            ...
            best_per_class_f1 = f1_score(y_val, y_pred, average=None, zero_division=0)
...
# 输出最优模型的Per-Class F1 Scores
print(f"\n  Per-Class F1-Scores for Best Model ({best_model_name}):")
print(f"  {'Class':<20} {'F1-Score':<12} {'Training Samples':<16}")
print(f"  {'-'*20} {'-'*12} {'-'*16}")
for i, class_name in enumerate(class_names):
    # 计算每个类别的训练样本数
    train_samples = np.sum(y_train == i)
    print(f"  {class_name:<20} {best_per_class_f1[i]:<12.4f} {train_samples:<16,}")
